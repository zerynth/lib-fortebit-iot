...
class MqttClient():
    ...
    def __init__(self, endpoint, device_token, ctx=None):
        ...
        self.ctx = ctx
        self.driver = mqtt.Client("polaris", True, 1000, 15000)
    ...
    def _loop_failure(self, client):
        while True:
            try:
                self.print_d("mqtt reconnecting...")
                client.reconnect()
                rc = client.get_return_code()
                if rc == mqtt.RC_ACCEPTED:
                    self.print_d("reconnected!")
                    break
                else:
                    self.print_d("mqtt rc =", rc)
            except Exception as e:
                self.print_d("_loop_failure",e)
            sleep(5000)
    ...
    def connect(self):
        ...
        try:
            # use keepalive period (PING packet) = 60 seconds
    ...
    def _on_attributes(self, client, payload, topic):
        self.print_d("> got:", topic, payload)
    ...
    def send_rpc_reply(self, id, result):
        self.print_d("rpc reply", id, result)
        try:
    ...
    def do_rpc_request(self, method, params=None, timeout=15000):
        self.rpc_id += 1
        self.rpc_ev.clear()
        self.print_d("rpc request:", self.rpc_id, method, params)
        try:
